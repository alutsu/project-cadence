...
| D35a | **S1's own result: implementing §5.1 literally made the game worse.** Runs went from clearing ~5 of 6 fights to 3.1, and socketed builders to 1.6–2.5. | The dominant variable is **deck size**, not Max HP. §5.1 starts a run at **5 cards**; M0's provisional deck was **12**, which is a *level-8* deck. The six encounters were authored and balanced against that 12-card deck, so **they are mid-run content being fought by a first-node character**. Even a curve calibrated exactly for §11's 16–20 fights reaches only deck 8–9 by fight six. The conclusion is not "speed up XP" — it is that **S1 cannot be validated on M0's chain at all**, because that chain is not a difficulty curve. It needs §11's map, §12.1's generation and §5.3's Threat to have a shape to be measured against. | here |
...
| **S3a** | 2 | **The real S1 measurement**, now that there is a run to make it against: win rate by Depth, and `ascetic` measured against every other builder | `focus` clears a full run in 20–40% of seeds, and **at least two builders beat `ascetic`** — the direct test of §5.1 `[FIX]`'s thesis that a growing pool makes the socket cost survivable |
